...
if __name__ == '__main__':
    ...
    ### and creating test user and test account for revenues to be attached to
    ###
    ...
    #Add an revenue to account
    #Test: test_rev_acct_class

    test_rev_acct_class = testAccount.createRev(revenue_display_name=("%s Rev-%s acct_class" % (account_display_number,random.randrange(111,999))),start_date=data.dtfunc('today','fulldate','dt'),end_date='',frequency=1,amount=420.69)

    if type(test_rev_acct_class)==ObjectId:
        test(name='test_rev_acct_class',test=1,outcome=True)
    else:
        test(name='test_rev_acct_class',test=1,outcome=False)

    #Add an revenue to account from revenue class
    #Test: test_rev_rev_class

    test_rev_rev_class = r.Rev.create(revenue_display_name=("%s Rev-%s rev_class" % (account_display_number,random.randrange(111,999))),start_date=data.dtfunc('today','fulldate','dt'),end_date='',frequency=1,amount=420.69,linked_account=account_creation)

    if type(test_rev_rev_class)==ObjectId:
        test(name='test_rev_rev_class',test=1,outcome=True)
    else:
        test(name='test_rev_rev_class',test=1,outcome=False)